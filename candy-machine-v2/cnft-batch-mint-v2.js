...
const { keypairIdentity, publicKey, percentAmount } = require('@metaplex-foundation/umi');
...
if (!addressFile) {
  console.error('Usage: node cnft-batch-mint-v2.js <addresses-file> [image-cid]');
...
// Create metadata JSON (not just image URL)
function createMetadata(index, treeInfo) {
  return {
    name: `Unify Promo NFT #${index}`,
    symbol: 'UNIFY',
...
async function batchMintCompressedNFTs() {
  ...
  console.log('BATCH MINTING COMPRESSED NFTS (V2 - WITH METADATA)');
  console.log('='.repeat(60));
  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    try {
      ...
      const metadata = createMetadata(i, treeInfo);
      ...
      console.log(`  ✅ Success`);
      successful++;
      results.push({ recipient, status: 'success', index: i });
  ...
  console.log('NFTs should appear in Phantom within 5-10 minutes\n');
}
