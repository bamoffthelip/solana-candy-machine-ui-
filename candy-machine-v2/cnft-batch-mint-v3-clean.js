...
if (!addressFile || !metadataFolderCID) {
  console.error('Usage: node cnft-batch-mint-v3-clean.js <addresses-file> <metadata-folder-cid>');
  console.error('\nExample:');
  console.error('node cnft-batch-mint-v3-clean.js addresses.txt bafybeiascws7hnrbos6f7nimovkhpmyyocpw26aof4tld2sjf6mj3qlwti');
  process.exit(1);
}

async function batchMintCleanCNFTs() {
  console.log('='.repeat(60));
  console.log('BATCH MINTING CLEAN CNFTS (V3 - CLEAN COLLECTION)');
  console.log('='.repeat(60));

  // Load tree info (v2 - clean)
  ...
  // Save results
  fs.writeFileSync('./batch-mint-results-clean.json', JSON.stringify(results, null, 2));
  console.log('Results saved to batch-mint-results-clean.json');
  console.log('\n✅ Clean cNFTs minted! Waiting for indexing...');
  console.log('   NFTs should appear in Phantom Collectibles within 10-15 minutes\n');
}

batchMintCleanCNFTs().catch(console.error);
